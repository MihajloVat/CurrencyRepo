...
function getMonthsDifference(lastDateStr) {
    ...
    const currentMonth = now.getMonth() + 1; // Місяці в Date починаються з 0

    if (lastDateYear === currentYear && lastDateMonth === currentMonth) {
        return 0;
    }

    const lastDate = new Date(lastDateYear, lastDateMonth - 1); // Місяці в JavaScript починаються з 0
